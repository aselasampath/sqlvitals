...
public class WaitStatsRepository(IConfiguration configuration) : BaseRepository(configuration), IWaitStatsRepository
{
    ...
    public async Task<IEnumerable<Recommendation>> GetRecommendationsAsync()
    {
        const string sql = """
            ...
            CategoryTotals AS (
                SELECT
                    SUM(CAST(wait_time_ms AS BIGINT)) AS TotalWaitMs,
                    SUM(CAST(signal_wait_time_ms AS BIGINT)) AS TotalSignalMs,
                    SUM(CASE WHEN wait_type IN ('SOS_SCHEDULER_YIELD','THREADPOOL','CMEMTHREAD','CXPACKET','CXCONSUMER','CXSYNC_PORT','CMEMPARTITIONED') THEN CAST(wait_time_ms AS BIGINT) ELSE 0 END) AS CPUWaitMs,
                    SUM(CASE WHEN wait_type IN ('ASYNC_IO_COMPLETION','IO_COMPLETION','PAGEIOLATCH_SH','PAGEIOLATCH_EX','PAGEIOLATCH_UP','PAGEIOLATCH_NL','PAGEIOLATCH_DT','PAGEIOLATCH_KP','WRITELOG','WRITE_COMPLETION','IO_QUEUE_LIMIT','IO_RETRY','BACKUPIO','BACKUPBUFFER','LOGMGR','LOGMGR_QUEUE','LOGMGR_FLUSH','LOG_RATE_GOVERNOR') THEN CAST(wait_time_ms AS BIGINT) ELSE 0 END) AS IOWaitMs,
                    SUM(CASE WHEN wait_type IN ('RESOURCE_SEMAPHORE','RESOURCE_SEMAPHORE_QUERY_COMPILE','PAGELATCH_SH','PAGELATCH_EX','PAGELATCH_UP','LATCH_SH','LATCH_EX','LATCH_UP','MEMORY_ALLOCATION_EXT','SOS_VIRTUALMEMORY_LOW') THEN CAST(wait_time_ms AS BIGINT) ELSE 0 END) AS MemWaitMs,
                    SUM(CASE WHEN wait_type LIKE 'LCK_M%' THEN CAST(wait_time_ms AS BIGINT) ELSE 0 END) AS LockWaitMs,
                    SUM(CASE WHEN wait_type IN ('ASYNC_NETWORK_IO','NET_WAITFOR_PACKET','NETWORK_IO','OLEDB') THEN CAST(wait_time_ms AS BIGINT) ELSE 0 END) AS NetWaitMs,
                    SUM(CASE WHEN wait_type IN ('CXPACKET','CXCONSUMER','CXSYNC_PORT') THEN CAST(wait_time_ms AS BIGINT) ELSE 0 END) AS ParallelismWaitMs
                FROM WaitData
            )
            SELECT rec.RecommendationId, rec.Category, rec.Severity, rec.Title,
                   rec.Description, rec.Action, rec.MetricValue, rec.Threshold, GETDATE() AS CaptureTime
            FROM CategoryTotals ct
            CROSS APPLY (VALUES
                (1,'CPU',CASE WHEN CAST(ct.TotalSignalMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>25 THEN 'CRITICAL' WHEN CAST(ct.TotalSignalMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>10 THEN 'WARNING' ELSE 'OK' END,'CPU Pressure (Signal Wait %)','Signal wait ratio indicates CPU scheduler contention.',CASE WHEN CAST(ct.TotalSignalMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>10 THEN 'Scale up CPU. Review MAXDOP. Identify CPU-heavy queries in Query Store.' ELSE 'No CPU action required.' END,CAST(CAST(ct.TotalSignalMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100 AS DECIMAL(10,2)),10.0),
                (2,'I/O',CASE WHEN CAST(ct.IOWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>40 THEN 'CRITICAL' WHEN CAST(ct.IOWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>20 THEN 'WARNING' ELSE 'OK' END,'I/O Bottleneck (PAGEIOLATCH/WRITELOG)','Disk subsystem cannot keep up with read/write demand.',CASE WHEN CAST(ct.IOWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>20 THEN 'Move data/log files to NVMe SSD. Enable Instant File Initialization. Add memory.' ELSE 'Monitor with sys.dm_io_virtual_file_stats.' END,CAST(CAST(ct.IOWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100 AS DECIMAL(10,2)),20.0),
                (3,'Memory',CASE WHEN CAST(ct.MemWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>30 THEN 'CRITICAL' WHEN CAST(ct.MemWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>15 THEN 'WARNING' ELSE 'OK' END,'Memory Pressure (RESOURCE_SEMAPHORE)','Queries queued waiting for memory grants.',CASE WHEN CAST(ct.MemWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>15 THEN 'Increase max server memory. Review dm_exec_query_memory_grants. Add indexes to reduce sort operations.' ELSE 'Monitor dm_os_memory_clerks.' END,CAST(CAST(ct.MemWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100 AS DECIMAL(10,2)),15.0),
                (4,'Locks',CASE WHEN CAST(ct.LockWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>20 THEN 'CRITICAL' WHEN CAST(ct.LockWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>5 THEN 'WARNING' ELSE 'OK' END,'Lock Contention (LCK_M waits)','Blocking and deadlock risk from long-held locks.',CASE WHEN CAST(ct.LockWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>5 THEN 'Enable READ_COMMITTED_SNAPSHOT. Review long transactions. Add covering indexes.' ELSE 'No lock action required.' END,CAST(CAST(ct.LockWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100 AS DECIMAL(10,2)),5.0),
                (5,'Network',CASE WHEN CAST(ct.NetWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>15 THEN 'WARNING' ELSE 'OK' END,'Network Bottleneck (ASYNC_NETWORK_IO)','Client cannot consume result sets fast enough.',CASE WHEN CAST(ct.NetWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>15 THEN 'Reduce result sets. Use pagination. Review SELECT * patterns.' ELSE 'No network action required.' END,CAST(CAST(ct.NetWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100 AS DECIMAL(10,2)),15.0),
                (6,'CPU',CASE WHEN CAST(ct.ParallelismWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>20 THEN 'WARNING' ELSE 'OK' END,'Excessive Parallelism (CXPACKET/CXCONSUMER)','Excessive parallel plans causing CPU thrashing.',CASE WHEN CAST(ct.ParallelismWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100>20 THEN 'Review MAXDOP. Set Cost Threshold for Parallelism to 50+.' ELSE 'CXPACKET waits acceptable.' END,CAST(CAST(ct.ParallelismWaitMs AS FLOAT)/NULLIF(ct.TotalWaitMs,0)*100 AS DECIMAL(10,2)),20.0)
            ) AS rec(RecommendationId,Category,Severity,Title,Description,Action,MetricValue,Threshold)
            ORDER BY CASE rec.Severity WHEN 'CRITICAL' THEN 1 WHEN 'WARNING' THEN 2 ELSE 3 END, rec.RecommendationId
            """;
        ...
        return rows.Select(r =>
        {
            string sev = (string)r.Severity;
            string color = sev == "CRITICAL" ? "#FF4560" : sev == "WARNING" ? "#FFA62B" : "#26E7A6";
            string icon  = sev == "CRITICAL" ? "🔴" : sev == "WARNING" ? "🟡" : "🟢";
            decimal mv = (decimal)r.MetricValue;
            decimal th = (decimal)r.Threshold;
            return new Recommendation(
                (int)r.RecommendationId, (string)r.Category, sev,
                (string)r.Title, (string)r.Description, (string)r.Action,
                mv, th, (DateTime)r.CaptureTime,
                color, icon, mv - th
            );
        });
    }

    // ── 7. Server health KPIs ─────────────────────────────────────────
    public async Task<ServerHealthKpi?> GetServerHealthKpiAsync()
    {
        const string sql = """
            SELECT si.sqlserver_start_time AS ServerStartTime, GETDATE() AS CaptureTime,
                DATEDIFF(HOUR, si.sqlserver_start_time, GETDATE()) AS UptimeHours,
                DATEDIFF(DAY,  si.sqlserver_start_time, GETDATE()) AS UptimeDays,
                si.cpu_count AS LogicalCPUs, si.hyperthread_ratio AS HyperthreadRatio,
                si.physical_memory_kb/1024 AS PhysicalMemoryMB,
                si.virtual_memory_kb/1024 AS VirtualMemoryMB,
                si.committed_kb/1024 AS CommittedMemMB,
                si.committed_target_kb/1024 AS TargetMemMB,
                ...
                (SELECT COUNT(*) FROM sys.dm_exec_requests WHERE status='running' AND session_id<>@@SPID) AS RunningRequests,
                (SELECT COUNT(*) FROM sys.dm_exec_requests WHERE wait_type IS NOT NULL AND wait_time>0 AND session_id<>@@SPID) AS WaitingRequests,
                CAST((SELECT SUM(CAST(signal_wait_time_ms AS FLOAT)) FROM sys.dm_os_wait_stats WHERE waiting_tasks_count>0)/NULLIF((SELECT SUM(CAST(wait_time_ms AS FLOAT)) FROM sys.dm_os_wait_stats WHERE waiting_tasks_count>0),0)*100 AS DECIMAL(10,2)) AS GlobalSignalWaitPct,
                CAST(ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Buffer cache hit ratio' AND object_name LIKE '%Buffer Manager%'),0) AS DECIMAL(10,2)) AS BufferCacheHitRatio,
                ISNULL((SELECT MIN(cntr_value) FROM sys.dm_os_performance_counters WHERE counter_name='Page life expectancy' AND object_name LIKE '%Buffer Manager%'),0) AS PageLifeExpectancy,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Batch Requests/sec' AND object_name LIKE '%SQL Statistics%'),0) AS BatchRequestsPerSec,
                -- New counters
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='SQL Compilations/sec'    AND object_name LIKE '%SQL Statistics%'),0)    AS SQLCompilationsPerSec,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='SQL Re-Compilations/sec' AND object_name LIKE '%SQL Statistics%'),0)    AS SQLRecompilationsPerSec,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Total Server Memory (KB)' AND object_name LIKE '%Memory Manager%'),0)   AS TotalServerMemKB,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Target Server Memory (KB)' AND object_name LIKE '%Memory Manager%'),0)  AS TargetServerMemKB,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Lock Waits/sec' AND object_name LIKE '%Locks%' AND instance_name='_Total'),0) AS LockWaitsPerSec,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Number of Deadlocks/sec' AND object_name LIKE '%Locks%' AND instance_name='_Total'),0) AS DeadlocksPerSec,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Transactions/sec' AND object_name LIKE '%Databases%' AND instance_name='_Total'),0)    AS TransactionsPerSec,
                ISNULL((SELECT TOP 1 cntr_value FROM sys.dm_os_performance_counters WHERE counter_name='Memory Grants Pending' AND object_name LIKE '%Memory Manager%'),0)     AS MemoryGrantsPending
        ...
        double signalPct = (double)(r.GlobalSignalWaitPct ?? 0);
        string cpuStatus = signalPct > 25 ? "CRITICAL" : signalPct > 10 ? "WARNING" : "NORMAL";
        string cpuColor  = cpuStatus == "CRITICAL" ? "#FF4560" : cpuStatus == "WARNING" ? "#FFA62B" : "#26E7A6";
        long   committed = (long)r.CommittedMemMB;
        long   target    = (long)r.TargetMemMB;
        double memPct    = target > 0 ? Math.Round((double)committed / target * 100, 1) : 0;
        long   ple       = (long)r.PageLifeExpectancy;
        string pleStatus = ple < 300 ? "LOW" : ple < 1000 ? "MODERATE" : "GOOD";


        return new ServerHealthKpi(
            (DateTime)r.ServerStartTime, (DateTime)r.CaptureTime,
            (int)r.UptimeHours, (int)r.UptimeDays,
            (int)r.LogicalCPUs, (int)r.HyperthreadRatio,
            (long)r.PhysicalMemoryMB, (long)r.VirtualMemoryMB,
            committed, target,
            (int)r.ActiveUserSessions, (int)r.BlockedRequests,
            (int)r.RunningRequests, (int)r.WaitingRequests,
            signalPct, (double)(r.BufferCacheHitRatio ?? 0),
            ple, (long)r.BatchRequestsPerSec,
            cpuStatus, cpuColor, memPct, pleStatus,
            (long)r.SQLCompilationsPerSec, (long)r.SQLRecompilationsPerSec,
            (long)r.TotalServerMemKB, (long)r.TargetServerMemKB,
            (long)r.LockWaitsPerSec, (long)r.DeadlocksPerSec,
            (long)r.TransactionsPerSec, (int)r.MemoryGrantsPending
        );
    }
    ...
    // ── 24. Utilization KPI (CPU ring buffer + memory counters) ──────
    public async Task<UtilizationKpi?> GetUtilizationKpiAsync()
    {
        // On-prem / MI: CPU from RING_BUFFER_SCHEDULER_MONITOR, workers from schedulers
        const string sqlOnPrem = """
            SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
            ;WITH cpu AS (
                SELECT CAST(rec.value('(./Record/SchedulerMonitorEvent/SystemHealth/ProcessUtilization)[1]','int') AS FLOAT) AS v
                FROM (
                    SELECT CONVERT(XML, record) AS rec
                    FROM sys.dm_os_ring_buffers
                    WHERE ring_buffer_type = N'RING_BUFFER_SCHEDULER_MONITOR'
                      AND record LIKE N'%ProcessUtilization%'
                ) t
            ), ranked AS (
                SELECT v, COUNT(*) OVER () AS n, ROW_NUMBER() OVER (ORDER BY v) AS rn FROM cpu
            ), cpu_summary AS (
                SELECT
                    ISNULL(AVG(v),  0) AS avg_cpu,
                    ISNULL(MAX(CASE WHEN rn >= CEILING(n * 0.95) THEN v END), 0) AS p95_cpu,
                    ISNULL(MAX(v),  0) AS max_cpu,
                    ISNULL(MAX(n),  0) AS samples
                FROM ranked
            )
            SELECT
                cs.avg_cpu  AS AvgCpuPct,
                cs.p95_cpu  AS P95CpuPct,
                cs.max_cpu  AS MaxCpuPct,
                cs.samples  AS SampleCount,
                si.cpu_count AS LogicalCpus,
                (SELECT SUM(current_workers_count) FROM sys.dm_os_schedulers WHERE status = N'VISIBLE ONLINE') AS WorkersUsed,
                (SELECT MAX(max_workers_count)     FROM sys.dm_os_schedulers WHERE status = N'VISIBLE ONLINE') AS WorkersTotal,
                ISNULL(MAX(CASE WHEN RTRIM(pc.counter_name) = N'Stolen Server Memory (KB)'  THEN pc.cntr_value END), 0) AS StolenMemKB,
                ISNULL(MAX(CASE WHEN RTRIM(pc.counter_name) = N'Total Server Memory (KB)'   THEN pc.cntr_value END), 0) AS TotalServerMemKB,
                ISNULL(MAX(CASE WHEN RTRIM(pc.counter_name) = N'Database Cache Memory (KB)' THEN pc.cntr_value END), 0) AS DatabaseCacheMemKB,
                si.physical_memory_kb   AS PhysicalMemoryKB,
                si.committed_target_kb  AS TargetMemKB
            FROM cpu_summary cs
            CROSS JOIN sys.dm_os_sys_info si
            CROSS JOIN sys.dm_os_performance_counters pc
            WHERE RTRIM(pc.object_name) LIKE N'%Memory Manager%'
              AND RTRIM(pc.counter_name) IN (
                N'Stolen Server Memory (KB)', N'Total Server Memory (KB)', N'Database Cache Memory (KB)'
              )
            GROUP BY cs.avg_cpu, cs.p95_cpu, cs.max_cpu, cs.samples,
                     si.cpu_count, si.physical_memory_kb, si.committed_target_kb;
            """;

        // Azure SQL DB: ring buffer and dm_os_sys_info not available;
        // use dm_db_resource_stats (up to 30 min at 15s intervals) for CPU
        const string sqlAzure = """
            SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
            ;WITH cpu AS (
                SELECT TOP 500 CAST(avg_cpu_percent AS FLOAT) AS v
            ...
            SELECT
                ...
                cs.max_cpu         AS MaxCpuPct,
                cs.samples         AS SampleCount,
                CAST(0 AS INT)     AS LogicalCpus,
                CAST(0 AS INT)     AS WorkersUsed,
                CAST(0 AS INT)     AS WorkersTotal,
                ISNULL(MAX(CASE WHEN RTRIM(pc.counter_name) = N'Stolen Server Memory (KB)'  THEN pc.cntr_value END), 0) AS StolenMemKB,
                ISNULL(MAX(CASE WHEN RTRIM(pc.counter_name) = N'Total Server Memory (KB)'   THEN pc.cntr_value END), 0) AS TotalServerMemKB,
                ISNULL(MAX(CASE WHEN RTRIM(pc.counter_name) = N'Database Cache Memory (KB)' THEN pc.cntr_value END), 0) AS DatabaseCacheMemKB,
                CAST(0 AS BIGINT)  AS PhysicalMemoryKB,
                CAST(0 AS BIGINT)  AS TargetMemKB
            FROM cpu_summary cs
            CROSS JOIN sys.dm_os_performance_counters pc
            WHERE RTRIM(pc.object_name) LIKE N'%Memory Manager%'
              AND RTRIM(pc.counter_name) IN (
        ...
        bool isAzureSqlDb = edition?.Edition == 5;

        var r = await QFirst(conn, isAzureSqlDb ? sqlAzure : sqlOnPrem);
        if (r is null) return null;
        return new UtilizationKpi(
            (double)r.AvgCpuPct,
            (double)r.P95CpuPct,
            (double)r.MaxCpuPct,
            (int)r.SampleCount,
            ...
            (int)r.WorkersTotal,
            (long)r.StolenMemKB,
            (long)r.TotalServerMemKB,
            (long)r.DatabaseCacheMemKB,
            (long)r.PhysicalMemoryKB,
            (long)r.TargetMemKB);
    }
    ...
    public Task<(
        SqlPulse.Engine.Models.PlanCacheHealthSummary Summary,
        IEnumerable<SqlPulse.Engine.Models.MultiPlanQuery> MultiPlanQueries,
        IEnumerable<SqlPulse.Engine.Models.KeyLookupQuery> KeyLookupQueries,
        IEnumerable<SqlPulse.Engine.Models.CardinalityMisestimate> CardinalityMisestimates,
        IEnumerable<SqlPulse.Engine.Models.SkewedParallelQuery> SkewedParallelQueries,
        IEnumerable<SqlPulse.Engine.Models.PlanGuide> PlanGuides
